...
class MoviesApiMixin(BaseListView):
    ...
    def annotate_filmwork(self, queryset):
        return queryset.annotate(
            ...
            actors=ArrayAgg(
                "personfilmwork__person__full_name", filter=Q(personfilmwork__role=RoleType.ACTOR), distinct=True
            ),
            directors=ArrayAgg(
                "personfilmwork__person__full_name", filter=Q(personfilmwork__role=RoleType.DIRECTOR), distinct=True
            ),
            writers=ArrayAgg(
                "personfilmwork__person__full_name", filter=Q(personfilmwork__role=RoleType.WRITER), distinct=True
            ),
        )
    ...
    def get_object(self):
        return (
            Filmwork.objects.filter(pk=self.kwargs["pk"])
