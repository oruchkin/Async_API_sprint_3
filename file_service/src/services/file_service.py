...
class FileService:
    ...
    async def upload_file(self, file: UploadFile, bucket: str, path: str) -> FileDbModel:
        ...
        id = uuid.uuid4()
        new_path = await self.generate_unique_path(self.db, str(id), file_extension)
    ...
    async def download_file(self, bucket, short_name: str) -> StreamingResponse:
        ...
        return await self.storage.get_file(bucket, file_db.path_in_storage)
    ...
    async def generate_unique_path(self, db: AsyncSession, base_name: str, file_extension: str) -> str:
        """Generate a unique path for a file with
        the same name already in the storage."""
        new_path = f"uploads/{base_name}{file_extension}"
        if await self.file_exists(db, new_path):
